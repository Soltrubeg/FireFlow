...
public class SetEntityGravityNode extends Node {
    ...
    public SetEntityGravityNode() {
        super("set_entity_gravity", "Set Entity Gravity", "Enables or disables gravity for an entity", Material.SHULKER_SHELL);
        ...
        signal.onSignal((ctx) -> {
            entity.getValue(ctx).use(ctx, e -> e.setGravity(enable.getValue(ctx)));
            ctx.sendSignal(next);
        });
    }
