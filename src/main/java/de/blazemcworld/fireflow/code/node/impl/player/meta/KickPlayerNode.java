...
public class KickPlayerNode extends Node {
    ...
    public KickPlayerNode() {
        super("kick_player", "Kick Player", "Forcefully makes a player leave the current space.", Material.BARRIER);
        ...
        signal.onSignal((ctx) -> {
            player.getValue(ctx).tryUse(ctx, p -> {
                if (ctx.evaluator.space.info.isOwnerOrDeveloper(p.getUniqueId())) {
