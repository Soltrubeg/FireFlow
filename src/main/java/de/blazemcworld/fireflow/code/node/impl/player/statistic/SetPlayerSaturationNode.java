...
public class SetPlayerSaturationNode extends Node {
    public SetPlayerSaturationNode() {
        super("set_player_saturation", "Set Player Saturation", "Sets the saturation of the player", Material.GLISTERING_MELON_SLICE);
        Input<Void> signal = new Input<>("signal", "Signal", SignalType.INSTANCE);
        Input<PlayerValue> player = new Input<>("player", "Player", PlayerType.INSTANCE);
        Input<Double> saturation = new Input<>("saturation", "Saturation", NumberType.INSTANCE);
        Output<Void> next = new Output<>("next", "Next", SignalType.INSTANCE);
        signal.onSignal((ctx) -> {
            player.getValue(ctx).tryUse(ctx, p -> p.setSaturation(saturation.getValue(ctx).floatValue()));
            ctx.sendSignal(next);
        });
    }
