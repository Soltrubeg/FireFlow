...
public class SetHeldSlotNode extends Node {
    public SetHeldSlotNode() {
        super("set_held_slot", "Set Held Slot", "Sets the held slot of the player", Material.GOLD_INGOT);
        Input<Void> signal = new Input<>("signal", "Signal", SignalType.INSTANCE);
        Input<PlayerValue> player = new Input<>("player", "Player", PlayerType.INSTANCE);
        Input<Double> slot = new Input<>("slot", "Slot", NumberType.INSTANCE);
        Output<Void> next = new Output<>("next", "Next", SignalType.INSTANCE);
        signal.onSignal((ctx) -> {
            player.getValue(ctx).tryUse(ctx, p -> p.getInventory().setHeldItemSlot(Math.clamp(slot.getValue(ctx).intValue() - 1, 0, 8)));
            ctx.sendSignal(next);
        });
    }
